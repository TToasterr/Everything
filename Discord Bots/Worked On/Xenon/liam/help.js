...
module.exports = {
        ...
        execute(message, args, client, time, final) {
                ...
                if (args == '') {
                        var general = [];
                        var commands = message.client.lCommands.map(command => command);

                        for (var i = 0; i < commands.length; i++) {
                                general.push(commands[i].name);
                        }

                        final.setTitle('__**Here are all of Liams commands!**__')
                                ...
                                .addField(`You can send \`rdl.help <command name>\` to get info on a specific command!`, '[`My Github`](https://github.com/TToasterr/Everything/tree/master/Discord%20Bots/Worked%20On/Xenon)');
                ...
                const command = message.client.lCommands.get(name);
                ...
                final.setTitle('**Specific Command Help**')
                        ...
                        .addField('**Usage**', `rdl.${command.name} ${command.usage}`);
