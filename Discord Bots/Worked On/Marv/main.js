...
client.on(`message`, message => {						// when the bot gets a message
	...
	var serverSettings = { SCPPrefix: true };				// the server has the SCP- prefix turned on by default
	...
	if (message.guild !== null) {						// if the message has a guild (not a DM)
		...
		console.log(`DM   ██  DM  ██  ${message.author.username}: ${message.content}`);
										// log it using only a DM instead of the guild name
	}
	...
	if (dm == false) {							// if its not a DM
		try {								// try to
			...
			serverSettings = { SCPPrefix: true };			// just use the default server settings (prefix = true)
		}
	}
	...
	if (message.content.toLowerCase().startsWith("marv.help")) {		// if the message starts with marv.help
		...
		final.setTitle('__**Here are all of my commands!**__')		// set the title of the embed
	        ...
	        .addField('General', '```\nhelp\ntoggleprefix\nstats```');	// add a field of all the commands
										...
										// if the message starts with marv.toggleprefix
		includesCommand = true;						// the message includes a command
			...
			for (var i = 5000; i >= 0; i--) {			// for every number between 1 and 5000
				if (!done) {					// if the SCP hasnt already been found
					...
					if (message.content.toUpperCase().includes(`${i}`)) {
										// if the message includes the number
						finalDesc.push(`[SCP-${i}](http://www.scp-wiki.net/scp-${i})`);
										// add the link to the SCP
	...
	if (dm == false) {							// if its not a DM
		...
		fs.writeFileSync(`./settings/${message.guild.name}.json`, JSONObject, (err) => {
										// write the serverSettings to the servers JSON file
