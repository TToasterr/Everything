...
module.exports = {
  ...
  execute(message, args, client, time) {
    ...
    try {
      fs.unlinkSync(`./commands/bank/${author}/${accountName}.json`, (err) => {
        if (err) {
          ...
          return console.log(`[${time}] ${message.author.username} tried to delete a bank account, but something went wrong.`);
        }
      });
    }
    catch (err) {
    ...
    message.channel.send(finalEmbed);
    return console.log(`[${time}] ${message.author.username} deleted a bank account.`);
  },
};
